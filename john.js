...
const corsOptions = {
  origin: [
    ...
    "http://192.168.43.250",
    "http://192.168.1.188:3001" // <-- Add this line
  ],
  ...
  async processTankData(data) {
    ...
    try {
      ...
      for (let tankNum = 1; tankNum <= ATG_CONFIG.MAX_TANKS; tankNum++) {
        ...
        const year = parseInt(timestamp.slice(0, 2));
        ...
        const timestampFormatted = `20${year.toString().padStart(2, '0')}-${validMonth.toString().padStart(2, '0')}-${validDay.toString().padStart(2, '0')} ${validHour.toString().padStart(2, '0')}:${validMinute.toString().padStart(2, '0')}:00`;

        console.log('Parsed timestamp components:', {
          raw: timestamp,
...
async function saveTankDataAsync(tankData) {
  try {
    ...
    console.log(`✅ Successfully saved data for tank ${tankData.tankNumber}`);
